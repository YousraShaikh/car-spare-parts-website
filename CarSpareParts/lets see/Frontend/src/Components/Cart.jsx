...
const Cart = () => {
  ...
  const handleRemoveItem = async (productId) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8000/api/v1/cart/remove-item`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="p-6 bg-gray-300 min-h-screen">
          ...
          {cartItems.map((item) => (
            ...
            >
              <div className="flex flex-col items-center animate-fadeIn">
                ...
                {/* Buttons - Decrement & Remove */}
                <div className="flex space-x-4">
                  <button
                    onClick={() => handleDecrement(item.product)}
                    className="bg-orange-500 text-white px-6 py-2 rounded-md hover:bg-orange-700 transition duration-300"
                  >
                    Decrement
                  </button>
      ...
      {cartItems.length > 0 && (
        <div className="flex justify-between items-center mt-8 text-gray-900">
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-300 p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-end space-x-2">
              <button
                onClick={() => setIsModalOpen(false)}
                className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-gray-600 transition duration-300"
              >
                Cancel
              </button>
              <button
